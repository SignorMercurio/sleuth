...
- `report_style.md`「成稿中的常见问题」：恢复 4d62e6d 误删的「本次」填充限制（替代写法 + 全文 ≤2 处），新增「支持……的判断」套语和一句多个分号两类；分号一条原本只在校验器里，writer 读不到。
- `report_writing_rules.md`：findings 字段名和记录动作不进正文；云端证据写成客户可读的可查时间范围，底稿没记录的首末时间不写；证据边界在其他章节默认省略，短指针全文 ≤2 处；报告引用的日志原文里攻击者 IoC 同样转义。措辞等级与证据用语不变。
- 报告模板（dossier 源头修改后同步）：去掉注释、占位和固定句里的「本次」「点亮」「纳入排查」等源头措辞，矩阵统一称「标出」；身份 `asset` 块无主体时整块删除、影响评估口径写成客户可读的限定、时间线偏移精确到分钟、时区只声明一次，均写入对应块注释。`findings_spec.md`、`attack_framework.md`、`verification_checklist.md` 同步改称「标出」。
- `curated-ir-excerpts.md`：去掉「只能说明……不能据此……」收尾和「本次 / 本报告」，改为写明还缺什么记录。
- `validate_full_reports.py`：可见正文禁止「点亮」；分号检查改为按句子和表格单元格判断。四份夹具由隔离 writer 按新规则重写并对照 findings 复读。
...
- `skills/sleuth/SKILL.md` 严禁状态变更：明确写 `/tmp`、`/dev/shm` 临时文件同样属于禁止的写盘，收敛实战会话中把枚举结果落到受害机再回读的做法。
- `skills/sleuth/references/report_writing_rules.md` 内部工作标识：取证工具名 SIREN 不进客户报告正文，改写为客户可理解的说法，收敛个别多主机报告出现「通过 SIREN 检查」的工具名外泄。
