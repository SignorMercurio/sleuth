...
- **调用云侧 skill**：按步骤 3.2 保持 `$sas` 告警、`sls` 已投递日志、只读 `opencli-aliyun-ir` 控制面/专用能力的优先级。只传 UID、站点/地域、资产标识、时间窗、待验证问题和必要 IoC；前一层覆盖不了才进入下一层。全部不可用则跳过并记录覆盖缺口，不直接执行 OpenCLI、SLS CLI、本地 shell 或 SSH。
