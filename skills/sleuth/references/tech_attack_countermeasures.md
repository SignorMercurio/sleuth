...
> ⚠️ **执行边界**：本文件区分两类命令。**检测/取证类**（`ls`/`cat`/`readelf`/`rpm -V`/`stat`/`netstat` 等只读命令）可经 SIREN 在受害主机执行；**处置/修复/安装类**（`rm`/`kill`/`chattr`/`reinstall`/`gcc`/`wget`/`curl … | bash` 等）一律「⚠️ 供客户执行，SIREN 不运行」——会破坏证据完整性、违反只读护栏，安装类还会在受害主机联网拉取并执行外部代码。这些处置步骤只作为客户处置建议。
