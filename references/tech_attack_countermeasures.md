...
> ⚠️ **执行边界**：本文件区分两类命令。**检测/取证类**（`ls`/`cat`/`readelf`/`rpm -V`/`stat`/`netstat` 等只读命令）可经 SIREN 在受害主机执行；**处置/修复/安装类**（`rm`/`kill`/`chattr`/`reinstall`/`gcc`/`wget`/`curl … | bash` 等）一律标注「⚠️ 供客户执行，SIREN 不运行」，**禁止经 SIREN 在失陷主机上执行**——它们会破坏证据完整性、违反只读护栏，安装类还会在受害主机上联网拉取并执行外部代码。这些处置步骤应写入报告的「响应行动」章节作为建议。
...
**方法 2: 使用 chkrootkit**（仅当主机已安装；不要在失陷主机上联网安装）
```bash
chkrootkit
# 重点关注 /etc/ld.so.preload 检查结果
```

**方法 3: 使用 unhide**（仅当主机已安装；不要在失陷主机上联网安装）
```bash
unhide proc
# 会列出被隐藏的进程
```
...
**使用 sysdig 抓取文件访问**（仅当主机已安装 sysdig；⚠️ 不要在失陷主机上 `curl … | bash` 联网安装）：
```bash
