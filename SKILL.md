...
主机侧日志常被清除/轮转/定位不到。需交叉验证攻击者真实 IP、攻击时间窗、命中的 WAF 规则、进程启动链、登录来源时，**调用 `sls` skill** 查阿里云云端日志（WAF / SAS / ActionTrail），作**补充证据源**（主线仍是 SIREN，仅在主机侧证据不足或需交叉验证时调用）。无跨 skill 调用工具则读 `sls` 指令或用户给的路径，仍不可用则跳过并在报告说明。
...
2. **读取写作规则**：读取 `references/report_style.md`（文风，含 `assets/style/` 样本读取规则）与 `references/report_writing_rules.md`（模板逐块填充 + 本项目特有约束）并执行
...
生成时先读取 `assets/report.md`，再编辑输出文件。保留 Markdown 标题、frontmatter 和 `:::` 指令块，只替换占位内容；默认写“客户阅读版”短报告：结论前置，证据够支撑结论就停止展开，除非用户明确要求深度取证版才加附录。**模板逐块怎么填**（frontmatter / 封面块 / 修订记录 / 事件概述 / 技术分析含 `::: attack` 矩阵 / 响应行动）见 `references/report_writing_rules.md`。
...
完整约束（结论可信度、未决旁支线索去向、云端证据写法、IoC 展示层转义、样式、篇幅与读者、命令证据、详略边界、文风）见 `references/report_writing_rules.md`「本项目特有约束」（步骤 8.2 已读取执行，以该文件为准）。其中两条不可让渡的红线务必落实：

- **结论可信度**：标为「已确认/确定」的结论必须已过步骤 7 验证，未过则降级为「推测/无法确认/未观察到」。
- **IoC 展示层转义**：报告可见文本里 IPv4 与攻击者域名最后一个点写 `[.]`、URL 协议头写 `hxxp://`，原始值仅用于命令执行/检索/内部分析。
