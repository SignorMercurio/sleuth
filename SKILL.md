...
- 仅缺其一 → 先问用户，拿不到则进入模式二
- **Client ID**（SIREN 客户端）缺失 → 用 `mcp__siren__ls` 列出客户端让用户选
...
### 1.1 确认 Client ID
`mcp__siren__ls()` — 未指定则展示列表请用户选。**退出条件**：无可用客户端 → 告知用户并结束。

### 1.2 【模式一】获取告警详情
`mcp__siren__get_alarm_detail(uid=<UID>, event_id=<Event ID>)` — 提取告警类型/级别、受影响资产、攻击特征、告警时间。
...
主机侧日志经常被清除、轮转丢失或定位不到。需要交叉验证攻击者真实 IP、攻击时间窗、命中的 WAF 规则、进程启动链、登录来源时，**通过 `Skill` 工具调用 `sls` skill**（或 Read `~/.agents/skills/sls/SKILL.md`）查询阿里云云端日志（WAF / 云安全中心 SAS / ActionTrail）。
