...
description: SLEUTH · 安全应急响应专家：通过 SIREN 在受害主机执行只读远程溯源调查，构建 ATT&CK 攻击链并生成中文应急响应报告。
  ...
  '这几台主机都查一下'、'合并报告'、'事件溯源'、'排查 webshell'、'查挖矿'、'排查反弹 shell'、'查暴破'、
...
当某条命令/查询会返回大输出（大日志、全盘 `find`、SLS 大结果集）时，优先派一个子 agent 去读原始输出、只回传结论 + IoC + 决定性的几行，保持编排者上下文精简（见「运行环境兼容」的派生子 agent 映射）。运行时不提供子 agent、或子 agent 拿不到 SIREN 时，按只读护栏内联降级：先 `wc -l` 评估，再 `head`/`tail`/`grep` 收窄。
...
- **多主机委托**：用户点名多台主机 / 多个 Client，或告警显示多资产受影响 → 步骤 1 确定主机清单，步骤 2–7 逐台执行（见工作流程概览）
...
**映射要求**：使用具体子技术编号；每条映射必须有证据支撑；未涉及的战术在文字映射里省略。注意这只针对文字映射——报告里的 `::: attack` 矩阵版式固定，不删战术（见步骤 8）。
...
> 子 agent 派生与降级规则见「运行环境兼容」「执行约定 · 重输出隔离」。

---

## 步骤 8：生成 Markdown 应急响应报告

报告模板位于 `<skill_root>/assets/report.md`，来源于 `dossier/report.md`。每次委托只在当前工作目录生成**一份**命名后的 Markdown 报告。

写作只以 findings 文件为事实来源；输入文件清单与禁止事项（不得改变事实与措辞等级、证据缺口处理）以 `references/findings_spec.md`「写作层使用规则」为准。写作可按「重输出隔离」模式交给子 agent 执行——只传入该规则列出的文件路径，不传调查上下文，写作子 agent 不碰 SIREN。

### 8.1 输出文件命名规范

文件名格式 `IR-{YYYYMMDD}-{hostname}-{event_type}[-{event_id}].md`。完整字段说明、事件类型 slug 对照表、多主机命名规则与示例见 **`references/report_naming.md`**——生成报告前读取以确定正确的 slug 与命名（本次会话已读过则复用，不重复读取）。
...
2. **读取风格指南**：读取 `references/report_style.md` 并执行（含 `assets/style/` 样本读取规则）
3. **拷贝模板**：将 `<skill_root>/assets/report.md` 复制为该输出文件
4. **填充报告副本**：只编辑该输出文件，把模板占位说明替换为本次事件的实际数据
5. **只交付 Markdown**：不要创建报告目录、`index.html`、CSS/JS、字体资源或 dev server；本 skill 的交付物是一个 `IR-….md` 文件（findings 工作底稿不算交付物）
